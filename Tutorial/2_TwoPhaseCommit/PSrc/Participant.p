  ...
  state WaitForRequests {
    ...
    on eReadTransReq do (req: tReadTransReq) {
      if(req.key in kvStore)
      {
        ...
        send req.client, eReadTransResp, (key = req.key, val = kvStore[req.key].val, status = SUCCESS, transId = kvStore[req.key].transId);
      }
      else
      {
        // read failed as the key does not exist
        send req.client, eReadTransResp, (key = "", val = -1, status = ERROR, transId = kvStore[req.key].transId);
      }
    }
